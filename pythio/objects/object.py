...
class Object:
    ...
    @staticmethod
    def wrap_list(expected_type, _list):
        ...
        for index, element in enumerate(_list):
            ...
            if isinstance(
                    expected_type, (type(List), type(List[str]))
            ) and isinstance(get_origin(expected_type), list):
                if isinstance(element, list):
                    _list[index] = Object.wrap_list(get_args(expected_type)[0], element)
                continue
            if not issubclass(expected_type, Object):
                continue
            if isinstance(element, expected_type):
                continue
            _list[index] = expected_type.wrap(element)
    ...
    def __init__(self, **kwargs):
        self.client: Optional['pythio.Client'] = None
        for key, value in kwargs.items():
            self[key] = value

    @staticmethod
    def unwrap_list(_list):
        _list = copy.copy(_list)
        for index, element in enumerate(_list):
            if isinstance(element, list):
                _list[index] = Object.unwrap_list(element)
            elif isinstance(element, Object):
                _list[index] = element.unwrap()
    ...
    def to_json(self):
        return json.dumps(self.unwrap(), ensure_ascii=False, indent=4)
